...
FTP , SSH , SMTP , and whois connections are allowed to be initiated out through the firewall, except that such connections must originate from the internal subnet on the subnet facing interface(eth1). HTTP , HTTPS, and DNS traffic is allowed through from any source because we need to alloww external address to interact with the internal web and DNS servers(after being NATed)

### 1.8.6 Network Address Translation

The final step in the construction of our iptables policy is to enable the translation of the non-routable 192.168.10.0/24 internal addresses into the routable externel 71.157.X.X address. This applies to inbound connections to the web and DNS servers from external clients, and also to outbound connections initiated from the systems on the internal network. For connections initiated from internal systems, we'll use the source NAT(SNAT) target, and for connections that are initiated from external systems, we'll use the destination NAT(DNAT) target.

The iptables nat table is dedicated to all NAT rules, and within this table there are two chains: PREROUTING and POSTROUTING . The **PREROUTING** chain is used to apply rules in the nat table to packets that **have not yet gone through the routing algorithm in the kernel in order to determine the interface on which they shhould be transmitted . Packets that are processed in this chain have also not yet been compared against the INPUT and FORWARD chains in the filter table**.

The **POSTROUTING** chain is responsible for processing **packets once they have gone through the routing algorithm in the kernel and are just about to be transmitted on the calculated physical interface. Packets processed by this chain have passed the requirements of the OUTPUT or FORWARD chains in the filter table (as well as requirments mandated by other tables that may be registered , such as the mangle table)** .
...
Referring to the network diagram in Figure 1-2, the IP address of the web and  DNS servers are 192.168.10.3 and 192.168.10.4 in the internal network. The three  PREROUTING rules allow web services and DNS requests from the external network to be sent to the appropriate internal servers. The final POSTROUTING rule allow connections that originate from internal non-routable network and destined for the external Internet to look as though they come from the IP adress 71.157.X.X

The vary last step in building the iptables policy is to enable IP forwarding in the Linux kernel:
...
### 1.8.7 Activating the Policy

```bash
# ./iptables.sh
```

### 1.8.8 iptables-save and iptables-resotre
...
### 1.8.9 Testing the Policy: TCP

It is most important to test the iptables policy from a host that is external to the local network , because this is the source of the majority of attacks (assuming a huge number of users are not on the internal systems). Effective testing is also important from the internal network , however, since one of the internal hosts could be compromised and then used to attack other internal hosts (including the firewall), even    though iptables is protecting the entire network.

开始测试时，先测试访问一个从内网和外网都不能访问到的TCP端口。RFC 793要求一个正确实现的TCP stack应满足：**如果一个SYN包到达一个关闭的端口，就会产生一个reset(RST/ACK)包**。关于RST包是否带有ACK标志位 将在第三章讨论。这提供给我们了一个测试iptables是否真正阻止了数据包的简单方法。因为如果尝试连接的返回包中没有RST/ACK数据包，这说明iptables借助内核接货了SYN包，且会阻止TCP stack给客户端返回RST/ACK。

随机选取TCP 5500端口，分别从内网和外网的主机进行测试。下面的例子展示了这个测试，结果显示iptables  的INPUT链确实是在正确的发挥作用，因为不仅仅是丢弃了数据包，还产生了相应的日志。在外网的ext_scanner上使用natcat连接防火墙的5500端口，正如期望的一样，netcat客户端hang住了，且在防火墙上产生了相应的日志：
...
> NOTE: 如果收到了RST/ACK数据包，netcat将会显示信息：**Connection refused**.

一个非常好的方法是采用Nmap对防火墙策略进行扫描。Nmap提供了许多不同的扫描类型，可用于辅助测试防火墙的connection-tracking and filtering capabilities.例如：

sending a surprise FIN packet (see Nmap’s -sF scanning mode) against a closed port
should not elicit a RST/ACK packet if iptables is working properly. Generating TCP
ACK packets that are not part of any established session (Nmap’s -sA mode) should
similarly be met with utter silence, because the connection-tracking subsystem is able
to discern that such packets are not part of any legitimate TCP session.给关闭的端口发送FIN包，在防火墙正常工作的情况下，是不会诱探出RST/ACK数据包的。产生的TCP ACK数据包如果不属于任何已建立连接的会话，则数据包会被丢弃。因为connection-tracking 子系统能分辨此数据包不属于任何合法的TCP会话。

### 1.8.10 Testing the Policy: UDP

基于UDP的socket连接与基于TCP的socket连接有很大不同。UDP是无连接协议，没有三次握手。数据的可靠传输可以通过基于UDP的应用层实现。

为展示iptables对UDP数据包的作用，从内外网分别发送数据包到UDP 5500端口。如果我们的UDP数据包没有被过滤，客户端应该收到一个ICMP Port Unreachable 信息。本次使用hping工具，命令如下：
...
> NOTE: 在这些测试中，任何没有特权的用户可以使用netcat来监听TCP或UDP的5500端口。但是我们完全不可能从iptables策略中没有明确允许通过的IP来访问服务器。这意味着运行与系统的服务不可能对系统的全局策略产生不利影响，在没有修改iptables策略的情况下。

### 1.8.11 Testint the Policy: ICMP

最后测试iptables对ICMP数据包的策略。iptables命令中使用了--icmp-type选项来限制可接受的ICMP数据包为Echo Request数据包。iptables会允许所有的Echo Requests数据包，其他的ICMP数据包应该被丢弃。测试时发送ICMP Echo Replay数据包而没有发送任何对应的Echo Request数据包，这会匹配到INPUT链开始的规则：INVALID state的匹配。本次继续使用hping工具分别在内外网进行测试：
